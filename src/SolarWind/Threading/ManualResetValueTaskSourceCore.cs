...
//TODO: Do not compile it for .netstandard 2.1 /.netcoreapp 3.0+
    ...
    public struct ManualResetValueTaskSourceCore<TResult>
    {
        ...
        public void OnCompleted(Action<object> continuation, object state, short token, ValueTaskSourceOnCompletedFlags flags)
        {
            ...
            if (oldContinuation != null)
            {
                ...
                switch (_capturedContext)
                {
                    case null:
                    {
                        WaitCallback callback = s => continuation(s);
        ...
        private void SignalCompletion()
        {
            ...
            if (_continuation != null || Interlocked.CompareExchange(ref _continuation, ValueTaskSourceHelper.s_sentinel, null) != null)
            {
                if (_executionContext != null)
                {
                    //TODO: Looks like a hack. Consider dropping this path completely. 
        ...
        private void InvokeContinuation()
        {
            ...
            switch (_capturedContext)
            {
                case null:
                {
                    //TODO: Completely undesired path due to redundant allocations
                    if (RunContinuationsAsynchronously)
                    {
                        ...
                        if (_executionContext != null)
                        {
                            ThreadPool.QueueUserWorkItem(callback, _continuationState);
                        }
                        else
                        {
                            ThreadPool.UnsafeQueueUserWorkItem(callback, _continuationState);
                        }
                    }
                    else
                    {
